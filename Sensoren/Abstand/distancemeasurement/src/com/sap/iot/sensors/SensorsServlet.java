...
public class SensorsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (action == null) {
			...
			for (int i = 0; i < sensors.size(); i++) {
				...
				List<Measurement> sensorMeasurements = dataHelper.getLastSensorReadings(sensor.getId(),
						MAX_NUMBER_SENSOR_READINGS);
	...
	private Measurement extractMeasurementData(HttpServletRequest request) {
		...
		if (sensorValueCalibration != null && sensorValueCalibration.length() > 0 && sensorValue != null
				&& sensorValueMultiplier != null && sensorValueMultiplier.length() > 0 && sensorValue.length() > 0) {
			measurement.setStoredAt(new Timestamp((new Date().getTime()) + 7200000));
	...
	public void init() throws ServletException {
		...
		try {
			...
			emf = Persistence.createEntityManagerFactory("distancemeasurement", properties);
